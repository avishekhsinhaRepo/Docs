...
public class Stockplex implements ComponentExporter{
    ...
    @ResourcePath(path = StockImportScheduler.STOCK_IMPORT_FOLDER)
    ...
    public void constructDataMap() {
        ...
        if(tradeValues != null) {
        	currentPrice = tradeValues.get(StockImportScheduler.LASTTRADE, Double.class);   	
            data.put("Request Date", tradeValues.get(StockImportScheduler.DAYOFUPDATE, String.class));
            data.put("Request Time", tradeValues.get(StockImportScheduler.UPDATETIME, String.class));
            data.put("UpDown", tradeValues.get(StockImportScheduler.UPDOWN, Double.class));
            data.put("Open Price", tradeValues.get(StockImportScheduler.OPENPRICE, Double.class));
            data.put("Range High", tradeValues.get(StockImportScheduler.RANGEHIGH, Double.class));
            data.put("Range Low", tradeValues.get(StockImportScheduler.RANGELOW, Double.class));
            data.put("Volume",  tradeValues.get(StockImportScheduler.VOLUME, Integer.class));
            data.put("Company", tradeValues.get(StockImportScheduler.COMPANY, String.class));
            data.put("Sector", tradeValues.get(StockImportScheduler.SECTOR, String.class));
            data.put("52 Week High", tradeValues.get(StockImportScheduler.WEEK52HIGH, Double.class));
            data.put("52 Week Low", tradeValues.get(StockImportScheduler.WEEK52LOW, Double.class));
        } else {
        	data.put(symbol,"No stock data imported for this stock symbol");
        }
    }
